...
void sem_literal_address(A_NODE *);
...
A_ID *getPointerFieldIdentifer(A_TYPE *, char *);
...
BOOLEAN isPointerOrArrayTypeSem(A_TYPE *);
...
{
    sem_program(node);
    sem_literal_address(node);
}

void sem_literal_address(A_NODE *node)
{
    int i;
...
A_TYPE *sem_expression(A_NODE *node)
{
    ...
    case N_EXP_ARROW:
        t=sem_expression(node->llink);
        id=getPointerFieldIdentifer(t,node->rlink);
        if (id)
        {
            result=id->type;
            if (!isArrayType(result))
    ...
    case N_ARG_LIST:
        ...
        else
        {
            if (id->type)
            {
                t=sem_expression(node->llink);
                node->llink = convertUsualUnaryConversion(node->llink);
                if (isAllowableCastingConversion(node->llink,id->type))
...
int sem_statement(A_NODE *node, int addr, A_TYPE *ret, BOOLEAN sw, BOOLEAN brk, BOOLEAN cnt)
{
    ...
    case N_STMT_LABEL_CASE:
        ...
        lit = getTypeAndValueOfExpression(node->line);
...
A_ID *getPointerFieldIdentifer(A_TYPE *t, char *s)
{
    A_ID *id = NIL;
...
BOOLEAN isPointerOrArrayTypeSem(A_TYPE *t)
{
