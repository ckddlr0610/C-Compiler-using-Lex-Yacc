...
void syntax_error(int i, char *s) {
    syntax_err++;
    printf("line no: %d, syntax error: , number: %d\n", line_no, i);
    switch (i)
    {
    case 11:
    ...
    case 24:
        printf("illegal declaration specifiers %s", s);
        break;

    case 25:
        printf("illegal function specifiers %s", s);
        break;
        ...
        makeIdentifier("malloc"),
         setTypeField(
            setTypeElementType(makeType(T_FUNC),string_type),
             setDeclaratorTypeAndKind(makeDummyIdentifier(),string_type,ID_PARM)),ID_FUNC);
}
